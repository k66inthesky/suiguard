...
export default function ComparisonCard({
  ...
  return (
    <div className="bg-white p-4 rounded-xl border border-gray-200 mb-2">
        ...
        <div key={pkgId} className="space-y-2">
          <div className={`p-3 rounded-lg border}`}>
            <div className="flex-col items-center gap-2">
              ...
              <div className="text-sm text-gray-600">版本號：{version ?? 'N/A'}</div>
      ...
      <button
        onClick={() => window.open(`https://suivision.xyz/package/${pkgId ?? ''}`, '_blank')}
        className="w-full mt-4 flex items-center justify-center gap-2 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors hover:cursor-pointer"
      >
