...
import { BLOCKLIST_URLS, FEATURES } from '../constants';
import { BlocklistData } from '../types';
...
export default function BlocklistPage() {
    ...
    return blocklist.some((item) => {
      ...
      if (typeof item === 'string') {
        ...
        if (type === 'domain') {
          ...
          match = normalizedItem === normalizedAddress || normalizedItem === packageIdFromAddress;
  ...
  const handleSearchClick = async () => {
    ...
    const addressType = (document.getElementById('addressType') as HTMLSelectElement).value;
    ...
    if (!blocklists || blocklists.totalRecords === 0) {
      setSearchResult('❌ Blocklist not loaded or failed to load, please restart the extension');
      return;
    }
    ...
    async function fetchBlocklists() {
      ...
      try {
        ...
        setBlocklists({
          ...
          domain,
          package: pkg,
          totalRecords: coin.length + object.length + domain.length + pkg.length,
        });
      } catch (e) {
        setBlocklists({
          coin: [],
  ...
  return (
    <PageLayout title={FEATURES.blocklist}>
      <div className="rounded-xl border border-gray-200 bg-white p-4">
        <div className="space-y-3">
          <div>
            <label htmlFor="addressType" className="mb-1 block text-sm font-medium text-gray-700">
              Address Type
            </label>
            <select
              id="addressType"
          ...
          <div>
            <label htmlFor="addressInput" className="mb-1 block text-sm font-medium text-gray-700">
              Enter address
            </label>
            <textarea
              id="addressInput"
          ...
          <button
            className="w-full hover:cursor-pointer rounded-xl bg-gradient-to-r from-blue-500 to-blue-700 px-4 py-3 font-medium text-white transition-all hover:from-blue-600 hover:to-blue-700 disabled:opacity-50"
